...
leftMotorSpeed = 67;        % Speed for left motor
...
adjustmentSpeed = 5;        % Speed adjustment for minor corrections
...
while true
    ...
    if manualControlFlag
        ...
        if distance > 30  % If there's plenty of space ahead (more than 30 cm)
            ...
            pause(0.3);  % Wait a little to get past the wall
            brick.StopMotor('AC', 'Brake');  % Stop both motors
              brick.MoveMotor(leftMotor, reverseSpeed);
            brick.MoveMotor(rightMotor, reverseSpeed);
            pause(0.58);
          
            brick.MoveMotor('C', -150);  % Move left motor backward
               brick.MoveMotor('A', 150);  % Move left motor backward
            pause(0.6);  % Turn left for 1.71 seconds (adjust time for sharpness)
            brick.StopMotor('C', 'Brake');
            brick.StopMotor('A', 'Brake');% Stop the left motor
            brick.MoveMotor('C', leftMotorSpeed);  % Move left motor forward
            brick.MoveMotor('A', rightMotorSpeed);  % Move right motor forward
            pause(3);  % Resume moving forward for 2 seconds
